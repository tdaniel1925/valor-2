...
| 4 | **Regression guard** | Eng | Default privileges are locked, but add a periodic check (CI or scheduled) that no `public` table grants anon/authenticated, so a future manual change can't silently reopen it. |
