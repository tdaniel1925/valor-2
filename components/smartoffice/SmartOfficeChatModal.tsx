...
import { Send, Sparkles, Loader2, ChevronDown, ChevronUp, Code, X, FileSpreadsheet, AlertCircle } from 'lucide-react';
...
interface SmartOfficeChatModalProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export default function SmartOfficeChatModal({ isOpen, onClose }: SmartOfficeChatModalProps) {
  ...
  const loadUploadedFiles = async () => {
    ...
    try {
      const response = await fetch('/api/smartoffice/files', {
        credentials: 'include',
      });
      ...
      if (response.ok) {
        ...
        // If API fails, fall back to checking database sync logs
        console.warn('Files API failed, files may be uploaded but API error occurred');
        setUploadedFiles([]);
      }
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-black bg-opacity-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-lg shadow-2xl w-full h-full max-w-7xl max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-6 py-4 flex items-center justify-between rounded-t-lg">
          <div className="flex items-center gap-3">
            <Sparkles className="w-6 h-6 text-white" />
            <div>
              <h3 className="text-xl font-semibold text-white">SmartOffice AI Assistant</h3>
              <p className="text-sm text-purple-100">Ask questions about your uploaded spreadsheets and SmartOffice data</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Uploaded Files Info Bar */}
        <div className="bg-blue-50 border-b border-blue-200 px-6 py-3">
          <div className="flex items-center gap-2 text-sm">
            <FileSpreadsheet className="w-4 h-4 text-blue-600" />
            {filesLoading ? (
              <span className="text-blue-600">Loading uploaded files...</span>
            ) : uploadedFiles.length > 0 ? (
              <span className="text-blue-700">
                <strong>{uploadedFiles.length}</strong> spreadsheet{uploadedFiles.length !== 1 ? 's' : ''} uploaded and processed
                {uploadedFiles.length > 0 && (
                  <span className="ml-2 text-blue-600">
                    (Latest: {uploadedFiles[0].name})
                  </span>
                )}
              </span>
            ) : (
              <span className="text-amber-600 flex items-center gap-2">
                ...
                No spreadsheets uploaded yet. Upload files to analyze your SmartOffice data.
              </span>
            )}
          </div>
        </div>
        ...
        {messages.length > 0 && (
          <div className="flex-1 overflow-hidden flex flex-col border-b border-gray-200">
            <button
              ...
              className="px-6 py-3 flex items-center justify-between hover:bg-gray-50 border-b border-gray-200"
            >
        ...
        <div className="p-6 bg-gray-50 rounded-b-lg">
          <div className="flex gap-3">
            <input
              ...
              disabled={loading}
              placeholder="Ask a question about your uploaded spreadsheets..."
          ...
          <p className="mt-2 text-xs text-gray-500">
            Powered by Claude AI • Results based on your uploaded SmartOffice spreadsheets and database
          </p>
        </div>
      </div>
    </div>
