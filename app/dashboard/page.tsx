    ...
    <AppLayout user={data.user}>
      <div className="w-full px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 sm:mb-8">
          {/* MTD Summary */}
          <div className="bg-blue-50 dark:bg-blue-900/30 rounded-lg shadow-lg p-4 sm:p-6 border border-blue-200 dark:border-blue-800">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-blue-900 dark:text-blue-100">Month-to-Date</h3>
              <div className="bg-blue-200 dark:bg-blue-800 bg-opacity-50 dark:bg-opacity-50 rounded-full p-2">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-blue-700 dark:text-blue-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* QTD Summary */}
          <div className="bg-purple-50 dark:bg-purple-900/30 rounded-lg shadow-lg p-4 sm:p-6 border border-purple-200 dark:border-purple-800">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-purple-900 dark:text-purple-100">Quarter-to-Date</h3>
              <div className="bg-purple-200 dark:bg-purple-800 bg-opacity-50 dark:bg-opacity-50 rounded-full p-2">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-purple-700 dark:text-purple-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {/* YTD Summary */}
          <div className="bg-green-50 dark:bg-green-900/30 rounded-lg shadow-lg p-4 sm:p-6 border border-green-200 dark:border-green-800">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-base sm:text-lg font-semibold text-green-900 dark:text-green-100">Year-to-Date</h3>
              <div className="bg-green-200 dark:bg-green-800 bg-opacity-50 dark:bg-opacity-50 rounded-full p-2">
                <svg className="w-5 h-5 sm:w-6 sm:h-6 text-green-700 dark:text-green-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {/* Quick Stats — book of business (SmartOffice source of truth) */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-6 sm:mb-8">
          {[
            { label: "Inforce Policies", value: (data.book?.inforce ?? 0).toLocaleString(), sub: "active in your book", color: "green" },
            { label: "Pending Policies", value: (data.book?.pending ?? 0).toLocaleString(), sub: "in progress", color: "amber" },
            { label: "Annual Premium", value: formatCurrencyCompact(data.book?.annualPremium ?? 0), sub: "total book", color: "blue", full: formatCurrency(data.book?.annualPremium ?? 0) },
            { label: "Commissionable Premium", value: formatCurrencyCompact(data.book?.commissionablePremium ?? 0), sub: "total book", color: "indigo", full: formatCurrency(data.book?.commissionablePremium ?? 0) },
          ].map((c) => (
            <Card key={c.label}>
              <CardContent className="p-4 sm:p-6">
                <p className="text-xs text-gray-500 dark:text-gray-400">{c.label}</p>
                <p
                  className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mt-2 tabular-nums whitespace-nowrap"
                  title={c.full ?? c.value}
                >
                  {c.value}
                </p>
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{c.sub}</p>
              </CardContent>
            </Card>
          ))}
        </div>
                ...
                {data.recentActivity.cases.length > 0 ? (
                  <div className="space-y-3 sm:space-y-4">
                    {data.recentActivity.cases.map((case_) => (
                      <div
                        key={case_.id}
                        className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 sm:p-4 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 gap-3 sm:gap-0"
                      >
          ...
          <CardContent className="p-4 sm:p-6">
            ...
            {data.recentActivity.commissions.length > 0 ? (
              <ResponsiveTable
                ...
                data={data.recentActivity.commissions}
              />
            ) : (
              <div className="text-center py-8">
