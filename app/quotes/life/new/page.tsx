...
import type {
  ...
  HealthClass,
  TobaccoUse,
} from "@/lib/integrations/winflex/types";
...
export default function NewLifeQuotePage() {
  ...
  const [quotes, setQuotes] = useState<WinFlexQuoteResponse | null>(null);
  ...
  const quoteMutation = useMutation({
    mutationFn: async (data: WinFlexQuoteRequest) => {
      const res = await fetch("/api/quotes/life", {
        method: "POST",
      ...
      return res.json() as Promise<WinFlexQuoteResponse>;
    },
    onSuccess: (data) => {
      setQuotes(data);
    },
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const request: WinFlexQuoteRequest = {
      applicant: {
      ...
      product: {
        ...
        term: formData.productType === "Term" ? parseInt(formData.term) : undefined,
  ...
  const emailMutation = useMutation({
    mutationFn: async (data: typeof emailFormData) => {
      ...
      const res = await fetch("/api/quotes/life/email", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      const res = await fetch("/api/quotes/life/pdf", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      a.href = url;
      a.download = `life-insurance-quotes-${Date.now()}.pdf`;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mb-8">
            ...
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Quote Results</h1>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {quotes.quotes.map((quote, index) => (
              <div
                key={quote.quoteId}
                className="bg-white rounded-lg shadow hover:shadow-lg transition-shadow border-2 border-gray-200 hover:border-blue-500"
              >
                {/* Best Value Badge */}
                {index === 0 && (
                  <div className="bg-green-500 text-white text-xs font-bold px-4 py-2 rounded-t-lg">
                    BEST VALUE
                  </div>
                )}
                ...
                <div className="p-6">
                  ...
                  <div className="mb-4 p-3 bg-gray-50 rounded-lg">
                    <div className="text-xs font-medium text-gray-600 mb-2">Financial Ratings</div>
                    <div className="flex gap-3 text-sm">
                      {quote.ratings.amBest && (
                        <div>
                          <span className="text-gray-600">A.M. Best:</span>
                          <span className="ml-1 font-semibold text-gray-900">{quote.ratings.amBest}</span>
                        </div>
                      )}
                      {quote.ratings.sp && (
                        <div>
                          <span className="text-gray-600">S&P:</span>
                          <span className="ml-1 font-semibold text-gray-900">{quote.ratings.sp}</span>
                        </div>
                      )}
                      {quote.ratings.moodys && (
                        <div>
                          <span className="text-gray-600">Moody's:</span>
                          <span className="ml-1 font-semibold text-gray-900">{quote.ratings.moodys}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">{formatCurrency(quote.annualPremium)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Coverage Amount:</span>
                      <span className="font-medium">{formatCurrency(quote.faceAmount)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Guaranteed Years:</span>
                      <span className="font-medium">{quote.guaranteedYears}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Underwriting:</span>
                      <span className="font-medium">{quote.underwritingType}</span>
                    </div>
                  </div>
                  ...
                  {quote.features && (
                    <div className="mb-4 p-3 bg-blue-50 rounded-lg">
                      <div className="text-xs font-medium text-blue-900 mb-2">Features</div>
                  ...
                  <div className="mb-4 text-xs text-gray-500">
                    Quote expires: {new Date(quote.expirationDate).toLocaleDateString()}
                  </div>
  ...
  return (
    <AppLayout>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">New Life Insurance Quote</h1>
          <p className="mt-1 text-sm text-gray-600">
            Get quotes from multiple top-rated carriers instantly
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                >
                  ...
                  <option value="Former">Former (quit 12+ months ago)</option>
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              {/* Term Length (only for Term products) */}
              {formData.productType === "Term" && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
              ...
              {/* Face Amount */}
              <div className={formData.productType === "Term" ? "" : "md:col-span-2"}>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    handleInputChange("faceAmount", value ? parseInt(value).toLocaleString() : "");
                  }}
                  placeholder="e.g., 500,000"
                  required
                />
                <p className="mt-1 text-xs text-gray-500">Minimum: $25,000</p>
              </div>
            </div>
          </div>
