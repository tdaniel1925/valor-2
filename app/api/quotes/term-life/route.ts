/**
 * API route for Term Life quote form submissions
 * POST /api/quotes/term-life
 */

import { NextRequest, NextResponse } from 'next/server';
import { resendClient } from '@/lib/integrations/resend/client';

interface TermLifeQuoteRequest {
  // Agent Information
  agentName: string;
  agentEmail: string;
...
function generateEmailHTML(data: TermLifeQuoteRequest): string {
  return `
<!DOCTYPE html>
<html>
<head>
  ...
  <title>Term Life Quote Request</title>
  <style>
    body {
...
<body>
  ...
  <div class="content">
    ...
    <div class="section">
      ...
      <div class="field">
        <div class="field-label">Term Duration:</div>
        <div class="field-value">${data.termDuration || 'Not specified'}</div>
      </div>
      <div class="field">
        <div class="field-label">Conversion Options:</div>
        <div class="field-value">${data.conversionOptions || 'Not specified'}</div>
      </div>
    </div>
    ...
    <div class="section">
      ...
      <div class="field">
        <div class="field-label">Medical Concerns:</div>
        <div class="field-value">${data.medicalConcerns || 'None provided'}</div>
      </div>
      <div class="field">
        <div class="field-label">Current Medications:</div>
        <div class="field-value">${data.currentMedications || 'None provided'}</div>
      </div>
    </div>
...
function generateEmailText(data: TermLifeQuoteRequest): string {
  return `
...
Term Duration: ${data.termDuration || 'Not specified'}
Conversion Options: ${data.conversionOptions || 'Not specified'}

HEALTH INFORMATION
------------------
Medical Concerns: ${data.medicalConcerns || 'None provided'}
...
export async function POST(request: NextRequest) {
  try {
    const body: TermLifeQuoteRequest = await request.json();
    ...
    const result = await resendClient.sendEmail({
      ...
      subject: `Term Life Quote Request - ${body.clientName || 'New Client'}`,
      html,
      text,
      tags: {
        type: 'term-life-quote',
    ...
    console.error('Error processing term-life quote request:', error);
    return NextResponse.json(
